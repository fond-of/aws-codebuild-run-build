...
describe('githubInputs', () => {
  ...
  it('build basic parameters for codeBuild.startBuild', () => {
    ...
    expect(test).to.haveOwnProperty('buildspecOverride').and.to.equal(undefined)
    expect(test)
...
describe('inputs2Parameters', () => {
  ...
  it('build basic parameters for codeBuild.startBuild', () => {
    ...
    expect(test).to.haveOwnProperty('projectName').and.to.equal(projectName)
    expect(test).to.haveOwnProperty('sourceVersion').and.to.equal(sha)
    expect(test).to.haveOwnProperty('sourceTypeOverride').and.to.equal('GITHUB')
    expect(test)
    ...
    expect(repoEnv).to.haveOwnProperty('name').and.to.equal('GITHUB_REPOSITORY')
